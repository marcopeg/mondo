...
export const CompanyTasksLinks = ({ file, config }: CompanyTasksLinksProps) => {
  const app = useApp();
  ...
  const tasks = useFiles(CRMFileType.TASK, {
    filter: useCallback(
      (candidate: TCachedFile, _app: App) => {
        ...
        // A company task references the company via the "company" property
        return matchesPropertyLink(candidate, "company", hostFile);
      },
      [hostFile]
    ),
  });
  ...
  const handleCollapseChange = useCallback(
    async (isCollapsed: boolean) => {
      ...
      try {
        ...
        console.error(
          "CompanyTasksLinks: failed to persist collapse state",
          error
        );
      }
    },
  ...
  const handleCreateTask = useCallback(async () => {
    ...
    try {
      ...
      console.error("CompanyTasksLinks: failed to create task", error);
    } finally {
      setIsCreating(false);
    }
  ...
  const companyName = getEntityDisplayName(file);
...
export default CompanyTasksLinks;
