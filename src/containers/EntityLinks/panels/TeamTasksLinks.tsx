...
export const TeamTasksLinks = ({ file, config }: TeamTasksLinksProps) => {
  const app = useApp();
  ...
  const tasks = useFiles(CRMFileType.TASK, {
    filter: useCallback(
      (candidate: TCachedFile, _app: App) => {
        ...
        // A team task references the team via the "team" property
        return matchesPropertyLink(candidate, "team", hostFile);
      },
      [hostFile]
    ),
  });
    ...
    async (isCollapsed: boolean) => {
      ...
      try {
        await app.fileManager.processFrontMatter(hostFile, (frontmatter) => {
          ...
          if (typeof frontmatter.crmState.tasks !== "object") {
        ...
        console.error(
          "TeamTasksLinks: failed to persist collapse state",
          error
        );
      }
    },
  ...
  const handleCreateTask = useCallback(async () => {
    ...
    try {
      ...
      if (taskFile) {
        // Set 'team' property to link back to the current team
      ...
      console.error("TeamTasksLinks: failed to create task", error);
    } finally {
      setIsCreating(false);
    }
