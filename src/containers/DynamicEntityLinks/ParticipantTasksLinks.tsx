...
const getTaskLabel = (task: TCachedFile): string => {
  const fm = task.cache?.frontmatter as Record<string, unknown> | undefined;
  const show = typeof fm?.show === "string" ? fm.show.trim() : "";
  if (show) return show;
  return task.file?.basename ?? "Untitled task";
};

const getTaskStatus = (task: TCachedFile): string | null => {
  const fm = task.cache?.frontmatter as Record<string, unknown> | undefined;
