...
export default class MyPlugin extends Plugin {
  ...
  async onload() {
    ...
    //   const div = el.createDiv({ cls: "my-plugin-footer" });
    //   div.setText("👋 Hello from plugin footer");
    // });
    // this.registerMarkdownPostProcessor((el, ctx) => {
    //   console.log("PostProcessor ran on:", ctx.sourcePath);
    ...
    this.registerEvent(
      this.app.workspace.on("active-leaf-change", (leaf) => {
        if (leaf?.view?.getViewType() === "markdown") {
          const view = leaf.view; // MarkdownView
